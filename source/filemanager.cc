...
extern "C" voidAPI write_to_file(file_t * file, const char * text)
{
...
extern "C" char * API read_from_file(file_t * file)
{
    FILE * file_ptr = (FILE*) DREF(file).file_stream;
...
extern "C" file_t * API create_file_manager(const char * file_path, const char * mode)
{
...
extern "C" void   API close_file_manager(file_t * file)
{
    fclose((FILE*) DREF(file).file_stream);
    free(file);
}

extern "C" void  API set_file_stream_position(file_t * file, unsigned int position)
{
    fseek((FILE*) DREF(file).file_stream, 0, SEEK_SET);
}

extern "C" void   API buffer_read_file(char * buffer, unsigned int size, file_t * file)
{
